...
export class VehicleComponent implements OnInit {
  ...
  savetrip(t_obj, tripInfo){
    this.dataService.saveTrip(t_obj, this.is_overlap, false, this.has_pax_limit).subscribe(res => {
      ...
      if(res['id']>0){ 
        ...
        if(tempTripInfo['driver_id']>0){
          const driver = this.dataService.getDriver(tempTripInfo['driver_id']);
          tempTripInfo['image_path'] = driver['image_path'];
        }else{
          tempTripInfo['image_path'] = '';
        }
       
        
        this.dataService.removeVehicleTrip(tripInfo['vehicle_id'], tripInfo['trip_id']);
        if(parseInt(tempTripInfo['vehicle_id'],10) >0){
  ...
  airportNotifier(event, callback):void{
    ...
    if(event['target'].parentNode.classList.contains('airport-trip')){
      ...
      }else if(tripinfo['driver_id']==='' || isNaN(tripinfo['driver_id']) ||  parseInt(tripinfo['driver_id'],10)<=0){
        this.message.open('Driver yet not assigned.', 'error');
      }else{
  ...
  refreshData(IsLoaderView){
    this.dataService.getVehicleTripByQuery(IsLoaderView, false);
  ...
  constructor(public dataService: DataService, public modalService: TripModalContent, 
    public airMailService: AirMailContent, public message: MessageComponent) {
  }
  ...
  ngOnInit() { 
    ...
    this.dataService.getConfiguration().subscribe(res=>{
        ...
        if(res!=null && res.hasOwnProperty('driver_categories')){
          this.dataService.driver_categories = res['driver_categories'];
        }
